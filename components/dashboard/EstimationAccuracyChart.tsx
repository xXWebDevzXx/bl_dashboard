...
export default function EstimationAccuracyChart({ className }: Props) {
  ...
  if (loading) {
    return (
      <div className={cn("bg-card border border-border-zinc/60 p-2 sm:p-4 desktop:p-6 rounded-sm shadow-xl shadow-black/25 animate-[fadeInScale_0.6s_ease-out_0.2s_both] overflow-hidden", className)}>
  ...
  return (
    <div className={cn("bg-card border border-border-zinc/60 p-2 sm:p-4 desktop:p-6 rounded-sm shadow-xl shadow-black/25 animate-[fadeInScale_0.6s_ease-out_0.2s_both] overflow-hidden", className)}>
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-4 mb-4 sm:mb-6">
        <div className="bg-card-foreground p-2 sm:p-3 desktop:p-4 rounded-lg border border-border-zinc/40 hover:border-emerald-normal/30 transition-all hover:shadow-lg hover:shadow-emerald-normal/10">
          <p className="text-[10px] sm:text-xs text-gray-400 mb-1 font-medium">AI Issues Accuracy</p>
          {data.aiTasks.taskCount > 0 ? (
            <>
              <p className="text-lg sm:text-xl desktop:text-2xl font-bold text-emerald-light">{data.aiTasks.accuracyPercentage.toFixed(1)}%</p>
        ...
        <div className="bg-card-foreground p-2 sm:p-3 desktop:p-4 rounded-lg border border-border-zinc/40 hover:border-cyan-normal/30 transition-all hover:shadow-lg hover:shadow-cyan-normal/10">
          <p className="text-[10px] sm:text-xs text-gray-400 mb-1 font-medium">Non-AI Issues Accuracy</p>
          {data.nonAiTasks.taskCount > 0 ? (
            <>
              <p className="text-lg sm:text-xl desktop:text-2xl font-bold text-cyan-light">{data.nonAiTasks.accuracyPercentage.toFixed(1)}%</p>
      ...
      {/* Bar Chart */}
      <div className="h-[180px] sm:h-[220px] desktop:h-[250px] w-full bg-card-foreground rounded-lg border border-border-zinc/60 p-2 sm:p-3">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
            ...
            <YAxis stroke="#9CA3AF" tick={{ fill: "#9CA3AF", fontSize: 12 }} width={35} label={{ value: "Hrs", angle: -90, position: "insideLeft", fill: "#9CA3AF", fontSize: 11 }} />
            <Tooltip
            ...
            />
            <Legend wrapperStyle={{ color: "#9CA3AF", fontSize: "12px", fontWeight: 500 }} />
      ...
      {/* Interpretation */}
      <div className="mt-3 sm:mt-4 p-2 sm:p-3 bg-card-foreground rounded-lg border border-border-zinc/40">
        <p className="text-[10px] sm:text-xs text-gray-400 leading-relaxed">{data.aiTasks.taskCount === 0 ? "ℹ Add estimated time to AI issues to see accuracy metrics" : data.aiTasks.accuracyPercentage < 100 ? "✓ AI issues are completed faster than estimated" : data.aiTasks.accuracyPercentage > 110 ? "⚠ AI issues are taking longer than estimated" : "✓ AI issue estimates are accurate"}</p>
      </div>
    </div>
  );
}
