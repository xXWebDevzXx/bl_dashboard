...
function ChartTooltipContent<TValue extends number | string | Array<number | string>, TName extends number | string>({
  active,
  payload,
  className,
  indicator = "dot",
  ...
  formatter?: (value: TValue, name: TName, item: unknown, index: number, payload: unknown) => React.ReactNode;
  color?: string;
  hideLabel?: boolean;
            ...
            return (
              <div key={item.dataKey} className={cn("[&>svg]:text-muted-foreground flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5", indicator === "dot" && "items-center")}>
                {formatter && item?.value !== undefined && item.name ? (
                  formatter(item.value, item.name as TName, item, index, item.payload)
                ) : (
                  <>
                    {itemConfig?.icon ? (
                      <itemConfig.icon />
    ...
    <div className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
      {payload
        ...
        .map((item, index) => {
          ...
          return (
            <div key={`${item.value}-${index}`} className={cn("[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3")}>
