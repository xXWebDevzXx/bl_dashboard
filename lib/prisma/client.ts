      ...
      if (typeof value === "object" && value !== null) {
        return new Proxy(value as object, {
          get(modelTarget: object, modelProp: string | symbol): unknown {
            const modelValue = (modelTarget as Record<string | symbol, unknown>)[modelProp];

            // If it's a query method, wrap it with retry logic
            if (typeof modelValue === "function") {
              return async (...args: unknown[]): Promise<unknown> => {
                try {
                  return await (modelValue as (...args: unknown[]) => Promise<unknown>).apply(modelTarget, args);
                } catch (error: unknown) {
                  const err = error as { message?: string };
                  // Retry on connection errors
                  if (err?.message?.includes("Engine is not yet connected") ||
                      err?.message?.includes("Response from the Engine was empty")) {
