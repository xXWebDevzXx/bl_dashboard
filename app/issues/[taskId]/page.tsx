...
export default async function TaskPage({ params }: TaskPageProps) {
  ...
  if (!task) {
    return (
      <div className="p-8">
        <Link href="/issues" className="text-blue-500 hover:underline flex items-center gap-2 mb-4 w-fit">
          <ArrowLeft size={16} />
          Back to issues
        </Link>
  ...
  return (
    <div className="p-8">
      <Link href="/issues" className="text-blue-500 hover:underline flex items-center gap-2 mb-6 w-fit">
        <ArrowLeft size={16} />
        Back to issues
      </Link>
        ...
        <div className="bg-[#1A1F26] p-6 rounded-sm">
          ...
          {timeComparison ? (
            <>
              ...
              <p className="text-sm text-gray-400 mt-1">of estimated time</p>
            </>
          ) : (
            ...
            {task.togglTimes.map((entry) => (
              <div key={entry.id} className="bg-[#0F1419] p-4 rounded border border-gray-800 hover:border-gray-700 transition-colors">
                ...
                <div className="text-sm text-gray-400 flex gap-4">
                  ...
                  <span>End: {formatDateTime(entry.stop)}</span>
                </div>
              </div>
            ))}
          </div>
